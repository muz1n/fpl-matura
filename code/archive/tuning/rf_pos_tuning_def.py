#!/usr/bin/env python3
"""
rf_pos_tuning_def.py

Leichtgewichtiges Hyperparameter-Tuning fuer DEF-Position RandomForestRegressor.
Reduziert Grid-Groesse um Experimente zu beschleunigen.

Verwendung:
    python code/rf_pos_tuning_def.py --season 2022-23 --start_gw 30 --end_gw 38

Hinweise:
- Nutzt Feature-Engineering von rf_pos_models.py wieder (compute_rolling_features, build_features).
- Fokussiert NUR auf DEF-Spieler.
...
def train_def_model(
    ...
    def_data = train_df[train_df["position"] == "DEF"].copy()
    if len(def_data) == 0:
        return None
    X_list, y_list = [], []
...
# Prognose
# -------------------------------------------------------------------------
def predict_def(model, test_df):
    def_data = test_df[test_df["position"] == "DEF"].copy()
    if model is None or len(def_data) == 0:
        return [], []
    y_true, y_pred = [], []
    for _, row in def_data.iterrows():
        feats = build_features(row)
...
def run_grid_search(train_df, test_df, param_grid):
    ...
    for i, combo in enumerate(combos, 1):
        ...
        y_true, y_pred = predict_def(model, test_df)
        if len(y_pred) == 0:
            print(
...
# -------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Lightweight tuning for DEF RF model")
    parser.add_argument(
    ...
    def_train = len(train_df[train_df["position"] == "DEF"])
    def_test = len(test_df[test_df["position"] == "DEF"])
    print("\nData split:")
    print(
        f"  Train GW {train_gw_min}-{start_gw-1}: rows={len(train_df)} DEF_rows={def_train}"
    )
    print(f"  Test  GW {start_gw}-{end_gw}: rows={len(test_df)} DEF_rows={def_test}")

    if def_train == 0 or def_test == 0:
        print("Insufficient DEF data for training or testing.")
        return
