...
def train_fwd_model(
    ...
    fwd_data = train_df[train_df["position"] == "FWD"].copy()
    if len(fwd_data) == 0:
        return None
    X_list, y_list = [], []
...
def predict_fwd(model, test_df):
    ...
    if model is None or len(fwd_data) == 0:
        return [], []
    y_true, y_pred = [], []
    for _, row in fwd_data.iterrows():
        feats = build_features(row)
...
def run_grid_search(train_df, test_df, param_grid):
    ...
    for i, combo in enumerate(combos, 1):
        ...
        y_true, y_pred = predict_fwd(model, test_df)
        if len(y_pred) == 0:
            print(
...
# -------------------------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Lightweight tuning for FWD RF model")
    parser.add_argument(
    ...
    fwd_train = len(train_df[train_df["position"] == "FWD"])
    fwd_test = len(test_df[test_df["position"] == "FWD"])
    print("\nData split:")
    print(
        f"  Train GW {train_gw_min}-{start_gw-1}: rows={len(train_df)} FWD_rows={fwd_train}"
    )
    print(f"  Test  GW {start_gw}-{end_gw}: rows={len(test_df)} FWD_rows={fwd_test}")

    if fwd_train == 0 or fwd_test == 0:
        print("Insufficient FWD data for training or testing.")
        return
