...
def map_pos(v):
    try:
        v = int(v);  return {1:"GK", 2:"DEF", 3:"MID", 4:"FWD"}.get(v)
    except:
        s = str(v).strip().upper()
        return {"GK":"GK","GKP":"GK","DEF":"DEF","D":"DEF","MID":"MID","M":"MID","FWD":"FWD","ST":"FWD","F":"FWD"}.get(s)
...
def to_float(series): return pd.to_numeric(series, errors="coerce")
def per90(col, minutes):
    m = minutes.copy();  m = m.where(m>0, np.nan);  return col/(m/90.0)

def bench_reserve_cost():
    base = (BENCH.get("GK",0)*BENCH_MIN["GK"] + BENCH.get("DEF",0)*BENCH_MIN["DEF"] +
            BENCH.get("MID",0)*BENCH_MIN["MID"] + BENCH.get("FWD",0)*BENCH_MIN["FWD"])
...
if not all([name_col, pos_col, price_col]):
    raise ValueError("CSV braucht name/web_name, element_type/position, now_cost/price/value")
...
thr    = to_float(df["threat"])         if "threat" in df.columns else pd.Series(0, index=pool.index, dtype=float)
cre    = to_float(df["creativity"])     if "creativity" in df.columns else pd.Series(0, index=pool.index, dtype=float)
inf_   = to_float(df["influence"])      if "influence" in df.columns else pd.Series(0, index=pool.index, dtype=float)
...
pool["g90_pct"]   = pct_rank(g90);  pool["a90_pct"] = pct_rank(a90)
pool["cs90_pct"]  = pct_rank(cs90); pool["gc90_pct"]= pct_rank(gc90); pool["sv90_pct"]=pct_rank(sv90)
...
def minimal_cost_for(need, used_names, club_count, enforce_club, sorted_pool):
    ...
    for pos, k in need.items():
        if k<=0: continue
        cand = sorted_pool[(~sorted_pool["name"].isin(used_names)) & (sorted_pool["pos"]==pos)]
        if enforce_club:
            tmp = []
            cc = club_count.copy()
            for _, r in cand.sort_values("price").iterrows():
                if cc[r["team"]] >= MAX_PER_CLUB: continue
                tmp.append(float(r["price"])); cc[r["team"]] += 1
                if len(tmp)==k: break
            rem += sum(tmp)
        else:
...
def build_team(pool_df):
    ...
    for _, r in sorted_pool.iterrows():
        ...
        if can_add(r):
            picked.append({"name":r["name"],"team":r["team"],"pos":r["pos"],
...
for need_pos in order:
    if bypos[need_pos]:
        p = bypos[need_pos].pop(0); used.append(p)
        print(f"- {need_pos}: {p['name']} ({p['team'] if enforce_club else '-'}) £{p['price']:.1f}")
    else:
        print(f"- {need_pos}: —")
...
for pos in ["GK","DEF","MID","FWD"]:
    ...
    if cand:
        ...
        print(f"- {pos}: {b['name']} ({b['team'] if enforce_club else '-'}) £{b['price']:.1f}")
    else:
        print(f"- {pos}: —")
