...
def train_position_models(
    ...
    for pos in positions:
        ...
        for idx, row in pos_data.iterrows():
            ...
            # Fallback: ältere Season-Dateien nutzen 'points' statt 'total_points'
...
def main():
    ...
    # Ensure position column (Fallback auf 'pos' falls vorhanden)
    if "position" not in df.columns:
        if "pos" in df.columns:
            print("Hinweis: 'position' Spalte fehlt, verwende 'pos' und benenne um...")
            df = df.rename(columns={"pos": "position"})
        else:
            print(
                "Error: 'position' column not found in data (auch keine 'pos' Alternative)"
            )
            return
    ...
    for gw in range(start_gw, end_gw + 1):
            ...
            pred_list.append(
                {
                    ...
                    "actual_points": float(
                        player_row.get("total_points", player_row.get("points", 0.0))
                    ),
                }
            )

        # Schreibe Vorhersagen als Objekt mit Metadaten und "players"-Key
        ...
        with open(pred_path, "w", encoding="utf-8") as f:
            json.dump(rf_pos_obj, f, indent=2, ensure_ascii=False)
