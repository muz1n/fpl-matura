...
## Nächste Schritte
- Bench-Policy festlegen und in Backtests konsistent anwenden (Startelf/Bank klar trennen).
- Gegnerstaerke-Features einfuehren (z. B. Team-Defensivmetriken, Elo, Quoten) und Auswirkung messen.
- Robustere Minuten-Proxy (z. B. rolling minutes + news-heuristic) testen.

## Fehler & Learnings
- Fehler: Merge-Kollision auf `team_short`; Fix: Duplikat-Spalte vor Merge entfernen, eindeutige Keys.
- Fehler: Verwechslung Trainings-CSV Pfad; Fix: zentrale Pfad-Konstanten und Checks.
- Learning: Zeitliche Splits strikt einhalten; alle rolling Features mit `shift(1)`.